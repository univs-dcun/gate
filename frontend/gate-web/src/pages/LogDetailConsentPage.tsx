/**
 * LogDetailConsentPage — 로그 상세 (개인정보 노출 동의 ON)
 * 기존 LogDetailPage와 동일하나 LogTableConsent를 사용해 매칭 이미지 컬럼 추가
 */
...
import LogTableConsent from '@/components/common/LogTableConsent';
import LogDetailPanel from '@/components/common/LogDetailPanel';
import type { LogModule, LogResult, DateRangeValue } from '@/components/common';
import { getLogs, toLogEntryWithImages } from '@/services/log';
import type { MatchingResultType, LogEntryWithImages } from '@/services/log';
...
export default function LogDetailConsentPage() {
  ...
  /* 인증방식 탭 (기본: 모든 방식) — 얼굴/손바닥 필터는 백엔드 modality 지원 시 API 연결 */
  ...
  const [selectedEntry,     setSelectedEntry]       = useState<LogEntryWithImages | null>(null);
  ...
  useEffect(() => {
    ...
    if (prev && selectedId && prev !== selectedId) {
      setModuleFilter(''); setResultFilter(''); setSearchInput(''); setSearchQuery(''); setPage(1);
    }
  }, [selectedId]);

  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      if (moduleRef.current && !moduleRef.current.contains(e.target as Node)) setModuleDropdownOpen(false);
      if (resultRef.current && !resultRef.current.contains(e.target as Node)) setResultDropdownOpen(false);
    }
  ...
  const { data: apiData, isLoading, refetch } = useQuery({
    queryKey:  ['logs-consent', selectedId, queryParams],
    queryFn:   () => getLogs(queryParams).then(r => r.data.data),
    staleTime: 0,
  });

  useEffect(() => { refetch(); }, [location.key]); // eslint-disable-line react-hooks/exhaustive-deps

  const logs         = (apiData?.contents ?? []).map(toLogEntryWithImages);
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={moduleRef}>
              <button className={[TRIGGER_BASE, 'min-w-[150px] whitespace-nowrap'].join(' ')} onClick={() => { setModuleDropdownOpen(o => !o); setResultDropdownOpen(false); }}>
              ...
              {moduleDropdownOpen && (
                <div className={DROPDOWN_BASE}>
                  <div className="px-2 py-1 mb-1"><span className="text-[13px] text-[#64748b] tracking-[-0.325px]">{t('logs.test_module')}</span></div>
                  {MODULE_ITEMS.map(m => (
                  ...
                  <div className="border-t border-[#e2e8f0] mt-1 pt-1">
                    <button className={[DROPDOWN_ITEM, 'text-[#757b80] font-normal text-[13px]'].join(' ')} onClick={() => { handleModuleFilter(''); setModuleDropdownOpen(false); }}>{t('common.all')}</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="relative" ref={resultRef}>
              <button className={[TRIGGER_BASE, 'min-w-[140px] whitespace-nowrap'].join(' ')} onClick={() => { setResultDropdownOpen(o => !o); setModuleDropdownOpen(false); }}>
              ...
              {resultDropdownOpen && (
                <div className={DROPDOWN_BASE}>
                  ...
                  <div className="border-t border-[#e2e8f0] mt-1 pt-1">
                    <button className={[DROPDOWN_ITEM, 'text-[#757b80] font-normal text-[13px]'].join(' ')} onClick={() => { handleResultFilter(''); setResultDropdownOpen(false); }}>{t('common.all')}</button>
                  </div>
                </div>
              )}
            </div>
