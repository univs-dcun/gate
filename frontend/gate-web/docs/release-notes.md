...
### 🔌 백엔드 응답 용어 변경 반영 (featureId / featureSeq)
- **특징점 관리**: `FeatureItemResponse` 용어 변경 정합 — 기존 `featureId`(정수) → `featureSeq`(일련번호·삭제/수정 키), 기존 `fid` → `featureId`(FID). `FeatureRow`·테이블·일련번호/FID 컬럼·CRUD 키 일괄 반영
- **로그**: `MatchLog`/`LogEntry`에 `featureSeq` 추가, 매핑 연결. 로그 상세 팝업 **'User ID' → 'Feature Seq'**(`featureSeq` 표시, 백엔드에서 `userId` 제거됨)
- **화면 키워드 'FID' → 'Feature ID'** 전체 교체 (로그/특징점 테이블·상세)

### 🎨 UI
- 로그 상세 팝업에 **메모(식별/설명 정보) 행 추가** — 시스템 상세 카드 내 전체 폭, 등록/검증/매칭 노출
...
### 🔌 백엔드(v3) 추가 연동
- **프로젝트 색상 태그(`colorTag`)**: 생성 시 팔레트 인덱스 기반 자동 배정, 리스트 아바타·GNB 프로젝트 셀렉터에 색상 표시
- **웹훅 설정** `GET/PUT/DELETE /api/v1/projects/{projectId}/webhook` 연동(`webhook.ts`) — URL·데모·API 전송 토글, 저장/삭제

### 🎨 UI 정합
- **로그 상세 다이얼로그**: 모든 아이콘을 Figma 에셋으로 교체(인라인 SVG 제거), 유사도 카드 회색, 인증방식 카드/성공 그린/라이브니스 결과값/실시간 촬영본 문구 정합 (Figma node 1874:3767 / 1874:3491)
- **1:1 사진인증 모듈 아이콘** 신규 디자인 교체 (node 1875:4998)
- **테이블 헤더 고정**(sticky + 언더라인) — 로그상세/특징점/프로젝트 리스트
- **대시보드**: 상태카드 툴팁, 인증방식 탭 손바닥 색, 일일통계 더보기 색(`#006FFF → #64748B`)
- **로고** GNB 사이즈 정합(134×20), 기능/결과 필터 색·타이틀·hover 전체폭, 메모 줄바꿈, 프로젝트 리스트 null `-` → 공백

### 🧹 정리
- 개발지원: 웹훅 **이력·테스트 탭 제거** → 웹훅 설정만 노출
