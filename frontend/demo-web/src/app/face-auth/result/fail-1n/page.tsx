...
function Fail1NView() {
  ...
  return (
    ...
    >
      ...
      {/* Photo + message */}
      <div className="bg-white flex flex-col items-center py-5 gap-2 w-full mb-4">
        <div className="relative w-[90px] h-[90px]">
          ...
          <div className="absolute bottom-0 right-0 w-[24px] h-[24px] rounded-[14px] bg-[#ef4444] border border-white flex items-center justify-center">
            <span className="text-white text-[14px] font-bold leading-none">x</span>
          </div>
        </div>
        <div className="flex flex-col items-center gap-1">
          <p className="text-[16px] font-semibold text-[#ef4444] tracking-[-0.4px] leading-[1.4] text-center">
            {livenessFailed && !isSystemError ? tx.livenessFailTitle : tx.matchFailed}
          </p>
      ...
      {/* Cards */}
      <div className="w-full px-5 pb-[100px] flex flex-col gap-4">
        <div
          ...
          {livenessFailed && !isSystemError ? (
            <>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-[14px] font-semibold text-[#334155] tracking-[-0.35px]">{authResult?.data?.matchingHistoryId ?? "—"}</span>
              </div>
              <div className="flex flex-col gap-2">
                ...
                <div className="bg-[#F2F4F5] rounded-[8px] px-3 py-2">
                  <p className="text-[14px] font-semibold text-[#475569] tracking-[-0.35px] leading-[1.4] break-all">{authResult?.data?.transactionUuid ?? "—"}</p>
                </div>
              </div>
            </>
          ) : (
            <>
              <div className="flex flex-col gap-2">
                <p className="text-[15px] font-semibold text-[#1e293b] tracking-[-0.375px] leading-[1.4]">
                  {isSystemError ? tx.systemErrorTitle : tx.unableToVerifyUser}
                </p>
                <div className="flex flex-col gap-1">
                  <p className="text-[13px] text-[#475569] tracking-[-0.325px] leading-[1.6]">
                    {isSystemError ? errorMessage : tx.scanMismatchMsg}
                  </p>
                  {errorCode ? (
...
export default function Fail1NPage() {
  return (
    <Suspense>
      <Fail1NView />
    </Suspense>
