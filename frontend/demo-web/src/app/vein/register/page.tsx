...
import { ChevronLeft, StickyNote, CheckCircle2, RefreshCw, Hand, CameraOff } from "lucide-react";
...
export default function VeinRegisterPage() {
  ...
  // 손바닥 감지 시뮬레이션: 카메라 켜진 후 2.5s → 감지 → 0.8s → 스캔 시작 → 3.5s → 완료
  useEffect(() => {
      ...
      timer = setTimeout(() => {
        ...
        if (video && canvas && video.readyState >= 2) {
          ...
          if (ctx) {
            ...
            try {
              sessionStorage.setItem("veinPalmPhoto", canvas.toDataURL("image/jpeg", 0.8));
            } catch {}
          }
        }
  ...
  const resetScan = () => setScanStatus("ready");

  const isReadyToRegister = scanStatus === "complete" && userMemo.trim() !== "";
  ...
  const getGuideMessage = () => {
    switch (scanStatus) {
      ...
      case "scanning":  return <><span className="text-[#0D9488]">{tx.veinExtractingBold}</span>{tx.veinExtractingSuffix}</>;
      case "complete":  return <><span className="text-emerald-600 font-bold">{tx.veinRecognizedBold}</span>{tx.veinRecognizedRegSuffix}</>;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-medium text-slate-800 animate-fade-in">
      ...
      <header className="px-4 h-16 flex items-center justify-between sticky top-0 z-40 bg-white border-b border-slate-100">
        ...
        <h2 className="font-semibold text-lg tracking-tight text-slate-900">{tx.veinRegPageHeader}</h2>
        <div className="w-10" />
      </header>
          ...
          <div className="relative bg-[#020617] rounded-[3rem] overflow-hidden shadow-2xl border-4 border-white aspect-[4/5]">
            ...
            {cameraError ? (
              ...
              <div className="relative w-full h-full flex items-center justify-center">
                ...
                {scanStatus === "complete" && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center z-30">
                    <div className="flex flex-col items-center animate-check-pop bg-white/5 backdrop-blur-xl px-12 py-8 rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
                      ...
                      <p className="text-white/70 text-[11px] mt-2 font-medium">{tx.veinExtractSuccess}</p>
                    </div>
                  </div>
                )}
                ...
                {/* 다시 촬영 버튼 */}
                {scanStatus === "complete" && (
                  <div className="absolute bottom-6 left-0 right-0 flex justify-center z-40 px-10">
                    <button
                      onClick={resetScan}
                      className="animate-retry-in w-full max-w-[210px] flex items-center justify-center gap-2.5 text-white font-bold text-[14px] bg-slate-800/80 backdrop-blur-md py-4 rounded-2xl border border-white/20 shadow-2xl active:scale-95 transition-all"
                    >
                      <RefreshCw className="w-4 h-4 text-teal-400" strokeWidth={3} />
                      <span>{tx.veinRetakeBtn}</span>
                    </button>
                  </div>
                )}
              </div>
        ...
        <section className="space-y-4">
          <div className="bg-white border border-slate-100 p-6 rounded-[2.5rem] shadow-sm space-y-6">
            <div className="flex items-center gap-2.5 mb-2">
              <div className="w-1.5 h-5 bg-[#2DD4BF] rounded-full" />
              <h5 className="font-semibold text-base text-slate-900">{tx.enterRegInfo}</h5>
            </div>

            <div className="group">
              <label className="text-[11px] font-bold text-slate-400 mb-2.5 block ml-1 uppercase tracking-wider">{tx.veinMemoLabel}</label>
              <div className="relative">
                <StickyNote className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
                <input
                  type="text"
                  value={userMemo}
                  onChange={(e) => setUserMemo(e.target.value)}
                  placeholder={tx.veinMemoPH}
                  className="w-full bg-slate-50 border-2 border-transparent focus:border-[#2DD4BF] rounded-2xl pl-12 pr-4 py-4 text-sm font-medium outline-none text-slate-900 transition-all"
                />
              </div>
            </div>
            ...
            <button
              ...
              onClick={() => router.push(`/vein/register/result?name=${encodeURIComponent(userMemo)}`)}
              className={`w-full font-bold py-5 rounded-[1.8rem] shadow-xl transition-all flex items-center justify-center gap-2 text-base ${
                isReadyToRegister
                  ? "bg-slate-900 text-white hover:bg-[#14b8a6] active:scale-[0.97]"
                  : "bg-slate-200 text-slate-400 cursor-not-allowed"
              }`}
            >
              <CheckCircle2 className="w-5 h-5" />
              <span>{tx.veinCompleteBtn}</span>
            </button>
          </div>
        </section>
