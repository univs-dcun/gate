...
- **앱 소스의 프로파일 yml(`backend/*/src/main/resources/application-*.yml`)에 DB 접속 정보(URL·계정·비밀번호)를 두지 않는다 (UG-307).** 값은 `${SPRING_DATASOURCE_URL}` 같은 기본값 없는 플레이스홀더만 허용하며, 실제 값은 compose 환경변수 또는 gate-config 가 준다. 주석으로 남기는 것도 금지 — 각 서비스의 `DatasourceCredentialGuardTest` 가 빌드에서 막는다. 로컬 실행은 `SPRING_DATASOURCE_URL/USERNAME/PASSWORD` 환경변수를 주거나, `src/main/resources` **밖**(예: `backend/{svc}/config/`)에 둔 파일을 `--spring.config.additional-location=file:./config/` 로 준다. `src/main/resources/application-local.yml` 은 가드가 막고 bootJar 에 실리므로 쓰지 않는다. 플레이스홀더에 기본값(`${X:기본값}`)을 두는 것도 금지다.
