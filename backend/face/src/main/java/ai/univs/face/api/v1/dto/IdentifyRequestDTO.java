...
public record IdentifyRequestDTO(
    ...
    public IdentifyInput toIdentifyInput() {
        return new IdentifyInput(
                this.branchName,
                this.faceImage,
                StringUtils.hasText(this.transactionUuid) ? this.transactionUuid : UUID.randomUUID().toString(),
                StringUtils.hasText(this.clientId) ? this.clientId : "SYSTEM",
                this.checkLiveness != null ? this.checkLiveness : true,
                this.checkMultiFace != null ? this.checkMultiFace : true);
    }
}
