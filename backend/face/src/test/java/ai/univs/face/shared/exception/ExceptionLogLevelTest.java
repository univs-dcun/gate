 ...
 * 스택트레이스로 남고 있었고, CustomFaceException 핸들러에는 <b>로그 호출이 아예 없었다</b>.
 *
...
class ExceptionLogLevelTest {
    ...
    void setUp() {
        ...
        MockHttpServletRequest request = new MockHttpServletRequest("POST", "/api/v1/face/verify");
    ...
    void 클라이언트_오류는_WARN() {
        handler.handleFaceCustomException(new CustomFaceException(ErrorType.NOT_FACE_IMAGE));

        ILoggingEvent event = onlyEvent();
        assertThat(event.getLevel()).isEqualTo(Level.WARN);
        assertThat(event.getThrowableProxy())
                .as("얼굴이 없는 사진은 클라이언트 입력 문제다. 우리 호출 스택 90줄을 남길 이유가 없다")
                .isNull();
        assertThat(event.getFormattedMessage())
                .contains("FACE-001")
                .contains("NOT_FACE_IMAGE");
    }
    ...
    void 자기_장애를_기록한다() {
        ...
        // 3xx/5xx 를 CustomFaceException(INTERNAL_SERVER_ERROR) 로 감싸 던지므로, ML 매처가
        // 전면 장애여도 face 로그에는 한 줄도 남지 않았다. gate 에는 @ResponseStatus(BAD_REQUEST)
        // 때문에 400 으로 도착했다 — 어느 서비스에서도 ERROR 가 남지 않는 구간이었다.
        handler.handleFaceCustomException(new CustomFaceException(ErrorType.INTERNAL_SERVER_ERROR));
    ...
    void 이미지_거부도_기록한다() {
        handler.handleInvalidFaceImageException(
                new InvalidFaceImageException(ErrorType.TOO_MANY_FACES));

        ILoggingEvent event = onlyEvent();
        assertThat(event.getLevel()).isEqualTo(Level.WARN);
        assertThat(event.getFormattedMessage()).contains("TOO_MANY_FACES");
    }
    ...
    void 모듈_오류도_같은_기준() {
        handler.handleInvalidFaceModuleException(
                new InvalidFaceModuleException("ML-500", "SERVER_ERROR", "module down"));
    ...
    void 요청_정보를_남긴다() {
        ...
        assertThat(onlyEvent().getFormattedMessage()).contains("POST /api/v1/face/verify");
    }
