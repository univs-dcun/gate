...
public class GetLivenessQrCodeUseCase {
    ...
    public QrCodeResult execute(Long accountId, String apiKey) {
        ...
        sdkQrCodeRepository.save(SdkQrCode.builder()
                .code(code)
                .token(token)
                .type(SdkQrCodeType.LIVENESS)
                .expiresAt(LocalDateTime.now(ZoneOffset.UTC).plusSeconds(qrCodeTokenExpiry / 1000))
                .isUsed(false)
                .build());
        byte[] qr = qrCodeService.generateQrCode("liveness", code);

        // 라이브니스 링크 생성
        String link = qrCodeService.buildUrl("liveness", code);
