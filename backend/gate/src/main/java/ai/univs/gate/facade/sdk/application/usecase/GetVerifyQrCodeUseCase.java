...
public class GetVerifyQrCodeUseCase {
    ...
    @Transactional
    public QrCodeResult execute(Long accountId, String apiKey, String faceId) {
        ...
        // 1:1 확인 QR 생성
        String token = jwtTokenProvider.createQrCodeTokenForVerifyByFaceId(accountId.toString(), apiKey, faceId);
        String code = UUID.randomUUID().toString();
        sdkQrCodeRepository.save(SdkQrCode.builder()
                .code(code)
                .token(token)
                .type(SdkQrCodeType.VERIFY)
                .expiresAt(LocalDateTime.now(ZoneOffset.UTC).plusSeconds(qrCodeTokenExpiry / 1000))
                .isUsed(false)
                .build());
        byte[] qr = qrCodeService.generateQrCode("verify", code);

        // 1:1 확인 링크 생성
        String link = qrCodeService.buildUrl("verify", code);
