...
public class GetCreateUserQrCodeUseCase {
    ...
    public QrCodeResult execute(Long accountId, String apiKey) {
        ...
        // 사용자 등록 QR 생성
        String token = jwtTokenProvider.createQrCodeTokenForCreatingUser(accountId.toString(), apiKey);
        String code = UUID.randomUUID().toString();
        sdkQrCodeRepository.save(SdkQrCode.builder()
                .code(code)
                .token(token)
                .type(SdkQrCodeType.CREATE_USER)
                .expiresAt(LocalDateTime.now(ZoneOffset.UTC).plusSeconds(qrCodeTokenExpiry / 1000))
                .isUsed(false)
                .build());
        byte[] qrCode = qrCodeService.generateQrCode("register", code);

        // 사용자 등록 링크 생성
        String link = qrCodeService.buildUrl("register", code);
