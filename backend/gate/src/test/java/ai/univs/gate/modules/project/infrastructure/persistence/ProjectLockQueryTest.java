 ...
 *
 * <p>{@code DeleteProjectUseCase} 의 단위 테스트는
 * {@code validateOwnershipForUpdate} 를 <b>부르는지</b>까지만 본다. 그 아래에서
 * {@code @Lock} 이 사라져도 목(mock)에는 아무 차이가 없어 전부 초록이다. 그런데 그
 * 애노테이션 한 줄이 없으면 이름만 {@code ForUpdate} 인 평범한 조회가 된다.
 *
 * <p>UG-302 때는 동시 재발급이 활성 키 2개를 만드는 것을 막는 것이 목적이었다. UG-312 에서
 * 재발급이 제거돼 지금 잠그는 쓰기 경로는 삭제뿐이고, UG-311 이 프로젝트 수정을 여기에
 * 합류시킬 예정이다 — 삭제와 수정이 경쟁하기 때문이다.
 *
 * <p>동시성 자체를 재현하려면 진짜 DB 와 스레드 두 개가 필요하다. UG-300 이 그 기반을 dev 에
 * 들였으므로 이제 가능하다 — UG-311 을 할 때 함께 넣는 것이 맞다. 그전까지는 잠금 <b>선언</b>을
 * 못박는 것이 실효 있는 최소 방어다.
 */
