...
| 6 | [게이트웨이 설정 소유권](#6-게이트웨이-설정-소유권) | 공통 레이어 파일과 라우트 섹션 구획 규칙 |
...
게이트웨이 설정(gate-config 레포)은 두 레이어로 나뉜다 (UG-248).

### 6.1 공통 레이어 — `gateway-server.yml` (스캐폴드 소유)

전 환경 공유 설정: default-filters, CORS 헤더/메서드, eureka, management.
레포 분리(로드맵 ④) 시 이 파일이 스캐폴드 레포의 게이트웨이 템플릿으로 이동한다.

### 6.2 환경 레이어 — `gateway-server-{env}.yml`

환경 고유 값만 가진다: 서버 포트/SSL, CORS 오리진, **라우트**, 로깅.

### 6.3 라우트 소유권 구획

Spring은 리스트 프로퍼티를 인덱스 단위(`routes[0].id` …)로 병합하므로,
같은 리스트를 두 프로퍼티 소스에 나눠 정의하면 정의가 뒤섞인다.
따라서 **라우트는 배포 단위당 단일 파일**에 두고, 파일 내부를 소유권 섹션으로 구획한다:

| 섹션 | 소유 | 라우트 예 |
|---|---|---|
| `[스캐폴드]` | msa-scaffold | AUTH, AUTH-REFRESH-TOKEN-CHANGE-PASSWORD, AUTH-ACTUATOR |
| `[범용 모듈]` | msa-scaffold(선택 모듈) | NOTIFY-WEB-SOCKET |
| `[제품 gate]` | gate 제품 | GATE-*, KYC-MESSAGES |
| `[역량 모듈]` | 생체인증 역량 | FACE-PROXY, MATCH-SERVER, FXP-PREPROCESS (onpremise) |

규칙:
- 새 라우트는 반드시 소유 섹션 안에 추가한다. 섹션이 없으면 이 표를 갱신하며 만든다.
- 스캐폴드 섹션의 라우트 변경은 계약 변경이다 (버전 갱신 동반).
- 리스트 병합 금지 규칙: `default-filters`/`allowedHeaders`/`allowedMethods`는 공통 레이어에만,
  `routes`/`allowedOrigins`/`allowedOriginPatterns`는 환경 레이어에만 정의한다.
- 레포 분리 후: 스캐폴드 릴리스가 "스캐폴드 섹션이 채워진 환경 파일 템플릿"을 제공하고,
  제품 저장소가 제품/역량 섹션을 append 한다.

알려진 경계 이슈: 온프레미스 라우트에 AuthenticationFilter 부재 (§1.4 참조).
...
| 계약 버전 | 날짜 | 내용 |
|---|---|---|
| 1.2.0 | 2026-07-31 | §6 게이트웨이 설정 소유권 계약 추가 (UG-248, 공통 레이어 분리) |
