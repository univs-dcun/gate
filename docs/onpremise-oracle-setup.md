...
| 지우는 것 | 남기는 것 |
|---|---|
| 두 이력 테이블의 행, 각 시도의 프로브 이미지, `VERIFY_IMAGE` 행의 신분증 이미지 | `biometric_feature` 행과 등록 이미지, `match_type = REGISTER` 인 잔존 행 |

`feature_image_path` 는 행 종류에 따라 처리가 갈린다 — 대개 살아 있는 특징점과 **같은 파일**을
가리키지만 `VERIFY_IMAGE` 행에서는 그 요청에서 올린 신분증 이미지다. 배치 크기·실패 시 동작은
`docs/history-retention.md` 참고.
